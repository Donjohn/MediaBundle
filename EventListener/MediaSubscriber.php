...
class MediaSubscriber implements EventSubscriber
{
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        if ($media instanceof Media) {
            $this->providerFactory->getProvider($media)->postLoad($media);
        }
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        $media = $args->getEntity();
        if ($media instanceof Media) {
            $this->providerFactory->getProvider($media)->prePersist($media);
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        $media = $args->getEntity();
        if ($media instanceof Media) {
            $this->providerFactory->getProvider($media)->postPersist($media);
        }
    }
    ...
    public function preUpdate(LifecycleEventArgs $args)
    {
        $media = $args->getEntity();
        if ($media instanceof Media) {
            $this->providerFactory->getProvider($media)->preUpdate($media);
        }
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        $media = $args->getEntity();
        if ($media instanceof Media) {
            $this->providerFactory->getProvider($media)->postUpdate($media);
        }
    }
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        $media = $args->getEntity();
        if ($media instanceof Media) {
            $this->providerFactory->getProvider($media)->preRemove($media);
        }
    }
}
